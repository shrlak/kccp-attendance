...
export function Dialog({ open, onOpenChange, title, children, wide }: DialogProps) {
  return (
    <RDialog.Root open={open} onOpenChange={onOpenChange}>
      ...
      <RDialog.Portal>
        <RDialog.Overlay className="fx-fade fixed inset-0 z-[1000] bg-black/45 backdrop-blur-[2px]" />
        ...
        >
          <div className="fx-pop max-h-[calc(100dvh-2rem)] overflow-y-auto rounded-2xl border border-border bg-surface p-6 shadow-2xl focus:outline-none sm:p-7">
          <div className="mb-5 flex items-start justify-between gap-4 border-b border-border pb-4">
            ...
            <RDialog.Close
              aria-label="Close"
              className="-mr-2 flex min-h-10 min-w-10 items-center justify-center rounded-lg text-subtle transition-[background-color,color,transform] duration-200 [transition-timing-function:var(--ease-out-soft)] hover:bg-surface-alt hover:text-text active:scale-90"
            >

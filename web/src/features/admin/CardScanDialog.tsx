...
export function CardScanDialog({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  async function pickFile(file: File | null) {
    ...
    try {
      ...
      toast({ title: (e as Error)?.message || t('admin.newfamily.scan.failed'), tone: 'err' })
      reset()
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && close()} title={t('admin.newfamily.scan.title')} wide>
      {phase === 'pick' && (
        <div className="flex flex-col gap-3">
          ...
          {/* accept="image/*" without `capture` so mobile offers both camera and photo
              library; desktop gets the regular file picker. */}
          <input
            ref={fileRef}
            type="file"
            accept="image/*"
      ...
      {phase === 'extracting' && (
        <p className="py-10 text-center text-sm text-muted">{t('admin.newfamily.scan.extracting')}</p>
      )}

      {phase === 'review' && (
        <>
          <p className="mb-3 text-sm text-muted">{t('admin.newfamily.scan.reviewHint')}</p>

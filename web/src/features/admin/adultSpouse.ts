...
// 세례여부는 그 줄에 적힌 그 사람의 것으로 채운다. 복사하지 않는 것이 셋 있다:
//   · **교우 등록번호** — 한 사람에게 발급된 번호라, 같은 값을 둘이 들면 그 번호로 사람을
//     찾을 수 없게 된다.
//   · **직장 또는 학교명** — 카드에 한 칸뿐이고 그건 본인 것이다.
//   · **이메일** — 스키마가 이미 그렇게 말하고 있다: adult.members의 `members_lower_idx`는
//     `lower(email)`에 걸린 **유니크 인덱스**다. 이메일은 이 시스템에서 사람을 가리키는
//     열쇠라(구글 로그인이 이메일로 members 행을 찾는다) 둘이 같은 값을 들 수 없고, 복사하면
//     **배우자 등록이 통째로 실패한다** — 본인은 이미 들어간 뒤라 배우자 한 사람만 조용히
//     빠진다. 빈 문자열도 그 인덱스에 들어가므로 ''를 보내도 마찬가지다. 그래서 아예 보내지
//     않는다 (서버는 `body[k]!==undefined`일 때만 그 칸을 쓰므로 열이 NULL로 남는다).
// 휴대폰은 복사한다. 카드에 번호가 하나뿐이므로 그 번호가 세대의 유일한 연결 고리이고,
// 없는 것보다 (고칠 수 있는) 같은 번호가 낫다. 전화에는 그런 인덱스가 없다.

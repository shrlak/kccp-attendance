...
- **조를 가르는 것은 부서와 교육 단계다** (`eduBuckets`). 1주차 주일에는 2주차만 들은 사람과
  ...
  2주차만 · 수강 완료), 일정이 끝난 뒤에도 같은 가름이 선다. **정할 것이 없으므로 배정 창에는
  갯수 입력이 없다** — 미리보기에 뜨는 부서×단계 목록이 곧 결과다.
...
- **조별 명단이 목록 위에 뜬다** (`groupByEduDongsan`). 카드의 배지는 "이 사람이 몇 동산인가"에
  답하지만 교육 시간에 필요한 것은 그 반대다 ("1동산은 누구누구인가"). 조는 이름 안의 숫자로
  세운다 — 글자만으로 세우면 10동산이 2동산 앞에 온다.

...
## 새가족 교육 동산 배정 (`features/admin/eduDongsan.ts` · `AdminNewFamilyEdu.tsx`)
- **매주 새로 도는 자리다.** 새가족 교육 탭에서 사람을 골라(카드마다 체크) 오른쪽 위 '동산
  배정'을 누르면, 고른 사람들이 무작위로 조에 나뉘어 `members.new_member_dongsan`에 적힌다.
...
- **배정 규칙은 아직 무작위 하나뿐이다.** 누구를 같은 조로 묶고 누구를 갈라놓을지가 정해지면
  `eduDongsan.ts`의 섞는 자리(`shuffled`)만 그 규칙으로 갈아 끼운다 — 부서 분리·조 인원·라벨·
  저장·화면은 그대로다.

...
## 학교로 좁혀 보기 — CMU · Pitt (`filters.ts` `schoolsOf`/`matchesSchool` · `AdminMembers.tsx`)
- **멤버 탭에만 있는 칩 한 줄**: 전체 · CMU · Pitt · 학교 미기재. 이름 검색과 **곱해지고**,
  아래 부서 섹션과도 곱해진다 (대학부 섹션 안의 CMU). 명단·숨긴 멤버·스태프 세 목록에 똑같이
  걸린다 — 한 목록만 좁히면 나머지 목록이 방금 고른 학교를 무시하는 것처럼 읽힌다.
- **공용 필터(`Filter`)에는 넣지 않았다.** 출석부·통계·오늘이 세는 것은 그 주일에 누가
  왔는가이고 학교는 그 질문의 칸이 아니다 — 그래서 `filterLog`은 학교를 모른 채 예전 그대로다
  (출석 줄에는 학교가 없으므로, 넣으려면 줄마다 명단을 되짚어야 한다). 명단을 학교로 갈라 보는
  자리는 멤버 탭 하나다.
- **학교는 부서가 아니라 사람의 칸에서 읽는다** (`schoolOf(m.school_or_work)`, 새가족 교육 동산
  배정이 쓰는 그 함수 그대로). 새 컬럼도 새 API도 없다 — 이미 적혀 있던 값을 읽는 것뿐이다.
  줄머리 표가 모자(GraduationCap)인 것도 위의 검색줄과 다른 종류의 가름이기 때문.
- **'학교 미기재'도 하나의 칩이다**: 세 칩의 인원을 더하면 전체가 되어야 한다. 빼 두면 CMU도
  Pitt도 아닌 사람이 어느 칩에서도 안 보인다.
- **고를 것이 없으면 줄이 없다** (`schoolsOf`): CMU도 Pitt도 없는 부(장년부)에서는 칩 줄 자체가
  뜨지 않는다. 칩 목록은 **검색 전 명단**에서 뽑는다 — 검색어를 치는 동안 칩이 사라졌다
  나타나면 고를 수가 없다.
...
- **학교는 적어 주기만 하고 가르지는 않는다** (`SchoolMark`, 새가족 교육 카드). 카드의 부서
  옆에 CMU · Pitt이 붙어 그 사람이 어느 학교인지가 바로 읽히지만, **조를 가르는 것은 부서와
  교육 단계뿐이다** — 학교는 이 시스템의 칸이 아니라 `school_or_work`에 손으로 적힌 말에서

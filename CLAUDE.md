...
- **파싱은 서버가 한다.** 시트에 붙이는 Apps Script(`Code.gs`)는 "바뀌었다"고 두드리기만 하고,
  서버가 공개 CSV(`export?format=csv`)를 직접 읽어 `sheetSync.ts`로 해석한다. 스크립트에 파서를
  두면 규칙을 고칠 때마다 시트마다 다시 붙여넣어야 하고 버전이 갈린다. **시트를 고쳐도 되지만
  파서를 고칠 때 시트를 손댈 일은 없다.**
...
- **부서는 시트 안에 없다** (동산 이름뿐). 여름 시트는 대학·청년부 합동 한 장이고 봄·가을에는
  부서마다 링크가 따로라, 소스를 등록할 때 사람이 `group`을 정해 준다 (합동이면 빈 문자열).
...
- **시트가 없는 학기에 동산지기가 출석을 적는 자리.** 로그인이 아니라 **링크가 신원**이고,
  링크 하나가 **동산 하나**를 가리킨다 (`config.dongsan_links` = `[{token,group,subgroup,
  createdAt}]`, `20260817`). 그 링크로 보이는 것은 그 동산 사람들의 이름과 그들의 **동산모임
...
- 드롭다운은 **O/X 둘뿐**이다: 표에서 출석은 "줄이 있다/없다"라 "안 왔다"와 "아직 안 적었다"를
  구별하지 못한다. 대신 그 주일에 O가 하나도 없으면 화면이 "아직 아무도 적히지 않았습니다"로
  알린다 (시트 파서의 *빈칸≠결석* 규칙은 시트 쪽에서 그대로 살아 있다).

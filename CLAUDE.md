...
- **조를 가르는 것은 부서와 교육 단계다** (`eduBuckets`). 1주차 주일에는 2주차만 들은 사람과
  미수강인 사람이 서로 다른 조로, 2주차 주일에는 1주차만 들은 사람과 미수강인 사람이 그렇게
  갈린다 — 두 규칙을 합치면 주차와 무관하게 **단계가 곧 조**이고(`eduStage`: 미수강 · 1주차만 ·
  2주차만 · 수강 완료), 일정이 끝난 뒤에도 같은 가름이 선다.
  미리보기가 곧 결과다 (부서×단계마다 인원과 **그 묶음이 받을 조 번호**, 둘 이상으로 쪼갤 때만
  `→ 1조 2 · 2조 2`처럼 나눗셈까지).
- **부서를 넘지 않는다** — 대학부는 대학부끼리, 청년부는 청년부끼리 (`membersByGroup`).
...
- **정하는 것은 '조 갯수' 하나 — 전체를 몇 조로 나눌지다** (`assignEduDongsan`의 `count`,
  배정 창의 숫자 칸). 한 묶음을 몇으로 쪼갤지가 아니라 **총합**이고, 그 수를 부서×단계 묶음에
  풀어내는 자리가 `allocateGroups`다: 묶음마다 한 조씩 깔고, **남는 조는 한 조에 사람이 제일
  많이 몰린 묶음부터** 준다 (`인원 / 지금 조 수`가 가장 큰 곳 — 큰 묶음이 먼저 쪼개져 조마다의
  인원이 전체에서 고르게 맞는다). 같은 값이면 앞 묶음이 먼저다 — 매주 같은 답이 나오도록.
  - **아래위 끝이 있다** (`eduGroupBounds`): 아래는 **묶음 수**(부서와 단계는 넘지 않으므로
    그보다 적게는 나눌 수 없다), 위는 **인원 수**(한 조에 한 명). 그 밖의 값은 끝으로 당겨
    쓰고 **칸에도 당겨진 값을 그대로 보여준다** — 3이 적혀 있는데 4조가 나오면 어디서
    어긋났는지 알 수 없다. 왜 그 사이인지는 칸 아래 한 줄로 적어 준다 (`groupsRange`).
  - 한 묶음이 둘 이상으로 갈릴 때 **비로소 아래의 기준(성비·나이·학교·전공·신앙)이 누가 어느
    쪽으로 갈지를 정한다** — 조가 하나뿐인 묶음에서는 고를 것이 없으므로 기준이 아무것도
    바꾸지 않는다.
...
- **조별 명단이 목록 위에 뜨고, 거기서 사람을 옮긴다** (`groupByEduDongsan` + `EduDongsanResult`).
  ...
  ("1조는 누구누구인가"). 조는 이름 안의 숫자로 세운다 — 글자만으로 세우면 10조가 2조
...
- **기준은 두 방향뿐이다** (`eduDongsan.ts` `RULES`): **흩는 힘**(그 값이 조마다 반반이 되게)과
  **모으는 힘**(값이 비슷한 사람을 같은 조로). 부서마다 어느 기준을 어느 방향으로 쓰는지가
  그 표이고, 기준이 없는 부서(EM 등)는 무작위 그대로다.
...
- **무게가 곧 우선순위다.** 흩는 힘은 사람 수(0·1·2…)로 세고, 모으는 힘은 **닮은 짝의 비율**
  ...
  이유다. **짝의 비율로 세는 이유는 빈칸 때문이다**: 짝 수를 그냥 세면 조가 커질수록 모으는 힘이

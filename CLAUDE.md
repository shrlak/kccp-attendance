...
- **새가족 등록의 필수 칸은 이름 하나뿐이다 — 키오스크에서도** (`KioskNewMemberDialog`). 카드
  사진 등록이 이미 그 규칙이었는데(위 항목) 같은 종이를 손으로 옮겨 적는 키오스크만 소속을
  요구하고 있었다. 그 칸이 정하는 것은 부서 하나이고 부서는 비었을 때 넣을 값이 이미 있으므로
  (`적는 사람의 부서 → 청년부`, 카드 쪽과 같은 `fallbackGroup`), 그 칸 때문에 사람을 명단에
  못 올릴 이유가 없다. **이름만 남긴 것**은 이름이 이 시스템의 신원이기 때문 — 카드 쪽이 빈
  이름을 자리표로 채우는 것은 종이가 곧 사라지기 때문이고, 키오스크에서는 적는 사람이 그 자리에
  서 있어 다시 물을 수 있다. 화면에도 한 줄로 적어 둔다 (`kiosk.newMember.optionalHint`) —
  비워도 되는지 몰라 붙들려 있는 것이 이 규칙을 바꾼 이유 그 자체다.
  - **장년부에는 그 하나마저 없다 — 필수 항목이 없다.** 그 부의 키오스크 등록은 세대 카드를

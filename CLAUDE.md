...
## 칩 줄 — 부서 · 처지 · 학교 (`filters.ts` · `TraitFilter.tsx` · `AdminMembers.tsx` · `AdminNewFamily.tsx`)
- **위에서 아래로 좁혀 간다**: 부서(전체 · 대학부 · 청년부 · 부서 미기재) → 처지(청년부에서만:
  전체 · 대학원생 · 직장인 · 기타) → 학교(전체 · CMU · Pitt · Duquesne · 기타, 청년부에서는
  처지와 곱해진다). 멤버 탭에서는

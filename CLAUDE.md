...
- **대학부에는 기준이 있다** (`eduDongsan.ts` `GROUP_RULES`): ① 성비를 5:5에 가깝게 ② Pitt과
  CMU를 반반으로 ③ 전공이 비슷한 사람을 같은 조로. **순서가 곧 우선순위**이고 무게로 표현했다
  (`W_GENDER 100` · `W_SCHOOL 10` · `W_MAJOR 1`) — 전공을 가장 가볍게 두는 이유는 그 기준만
  **방향이 반대**이기 때문이다: 성비와 학교는 흩는 힘, 전공은 모으는 힘이라 무게가 비슷하면
  전공이 같은 사람을 한 조에 몰면서 성비가 무너진다. 지금은 성비·학교를 똑같이 잘 맞추는 답이
  여럿일 때 그중에서 전공이 고르는 모양이다. **청년부는 아직 기준이 없어 무작위 그대로**이고,
  기준이 정해지면 그 표에 한 줄을 더한다.
- **푸는 방법은 무작위 시작 + 맞바꾸기다** (`balancedGroups`): 무작위로 자리를 잡고, 두 사람을
  ...
  것이 될지가 그 주의 운이다.
- **성별·학교·전공은 이미 있는 칸에서 읽기만 한다** (`eduDongsanTraits.ts`): `members.gender`와
  **`members.school_or_work` 한 칸**이다. 그 칸은 종이 카드의 '학교/직장 학과'를 손으로 옮겨
  ...
  화면에 그 인원을 적어 준다 — 안 적으면 "왜 성비가 안 맞지"가 된다.
- **조마다 구성이 한 줄로 붙는다** (`GroupComposition`: `남2·여3 · CMU2·Pitt3 · 의료·생명3`).
  기준대로 나뉘었는지를 눈으로 검산하는 자리다 — 숫자가 없으면 이름을 세어 확인하게 된다.

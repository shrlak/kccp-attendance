...
- **매주 새로 도는 자리다.** 새가족 교육 탭에서 사람을 골라(카드마다 체크) 오른쪽 위 '동산
  배정'을 누르면, 고른 사람들이 그 부서의 기준대로 조에 나뉘어 `members.new_member_dongsan`에
  적힌다 (대학부는 성비·학교·전공, 청년부는 아직 무작위 — 아래).
  **정하는 것은 동산 갯수 하나**이고, 조마다 몇 명이 되는지는 누르기 전에 미리 보여준다
  (`bucketSizes` — 앞쪽 조부터 한 명씩 더 가서 차이는 언제나 1명 이하). 무작위가 정하는 것은
  누가 어디로 가느냐뿐이라 미리보기와 결과가 어긋나지 않는다.
- **부서를 넘지 않는다** — 대학부는 대학부끼리, 청년부는 청년부끼리 (`membersByGroup`).
  그래서 조 이름에도 부서가 붙는다 (`대학부 1동산`): "1동산"만 적으면 두 부서의 1동산이 같은
  이름이 되어 명단을 읽는 사람이 어느 쪽인지 알 수 없다.

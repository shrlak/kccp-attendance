...
## 학교로 좁혀 보기 — CMU · Pitt (`filters.ts` · `GroupFilter.tsx`)
- **부서·동산 칩 아래 학교 칩 한 줄**: 전체 · CMU · Pitt · 학교 미기재. 그 줄은 `GroupFilter`를
  놓는 모든 탭에 함께 선다 (오늘 · 출석부 · 통계 · 새가족 · 새가족 교육). 세 가름은
  **곱해진다** — 대학부 × CMU.
- **학교는 부서가 아니라 사람의 칸에서 읽는다** (`schoolOf(m.school_or_work)`, 교육 동산 배정이
  쓰는 그 함수 그대로). 그래서 줄머리 표도 위의 두 줄과 다르게 모자(GraduationCap)다. 새 컬럼도
  새 API도 없다 — 이미 적혀 있던 값을 읽는 것뿐이다.
- **'학교 미기재'도 하나의 칩이다**: 세 칩의 인원을 더하면 전체가 되어야 한다. 빼 두면 CMU도
  Pitt도 아닌 사람이 어느 칩에서도 안 보여서 "이 사람은 어디 갔지"가 된다.
- **고를 것이 없으면 줄이 없다** (`schoolsOf`): CMU도 Pitt도 없는 부(장년부)에서는 칩 줄 자체가
  뜨지 않고, 한 학교뿐이어도 좁힐 것이 없어 뜨지 않는다.
- **부서를 바꿔도 고른 학교는 남는다** — 동산은 부서의 것이라 부서를 바꾸면 풀리지만, 학교는
  부서와 무관한 가름이다.
- **출석 줄에는 학교가 없다** (그건 사람의 칸이다). 그래서 `filterLog`은 학교 칩이 켜져 있을
  때만 명단을 받아 그 줄이 누구의 것인지 되짚는다 — **열쇠는 `memberId`**고(동명이인이 갈린다)
  그것이 없는 옛 줄만 이름으로 찾는다. 멤버로 이어지지 않는 줄(손님)은 학교를 물을 상대가 없어
  그동안 빠진다. 출석부 탭은 되짚을 명단에 **숨긴 멤버까지** 넣는다 — 같은 log을 아카이브가
  읽고, 끝난 학기의 출석부에는 그때 있던 사람이 다 들어 있어야 한다.
...
- **조를 가르는 것은 부서와 교육 단계다** (`eduBuckets`). 1주차 주일에는 2주차만 들은 사람과
  ...
  2주차만 · 수강 완료), 일정이 끝난 뒤에도 같은 가름이 선다. 배정 창의 **'한 단계를 몇 조로'**
  는 그 단계 **안에서** 다시 몇으로 쪼갤지이고 기본값이 1이다 — 단계를 넘어 섞이는 일은 없다.
  미리보기가 곧 결과다 (부서×단계마다 인원, 둘 이상으로 쪼갤 때만 `→ 2 · 2`처럼 나눗셈까지).
- **부서를 넘지 않는다** — 대학부는 대학부끼리, 청년부는 청년부끼리 (`membersByGroup`).
  **조 이름은 부서 + 단계다** (`대학부 미수강` · `청년부 2주차만`): 조를 가르는 것이 단계이므로
  번호보다 단계 이름이 그 조를 가리키는 말이고, 부서가 붙어 있어야 두 부서의 같은 단계가 한
  이름이 되지 않는다. 한 단계를 둘 이상으로 쪼갤 때만 뒤에 번호가 붙는다 (`대학부 미수강 2`).
- **학교로도 가를 수 있다 — 배정 창의 체크 한 칸** (`eduBuckets(members, bySchool)`,
  `admin.newfamilyEdu.assign.bySchool`). 켜면 부서와 단계 사이에 학교가 한 겹 더 들어가
  CMU · Pitt · **학교 미기재**가 서로 다른 조가 되고, 이름에 그 순서 그대로 적힌다
  (`대학부 CMU 미수강`). **기본값은 꺼짐**이고 끄면 예전과 한 글자도 다르지 않다 — 학교는 이
  시스템의 칸이 아니라 `school_or_work`에 손으로 적힌 말에서 읽어낸 값이라(`schoolOf`) 늘 맞는
  가름이 아니고, 켜는 순간 조가 세 배 가까이 는다. 못 읽은 사람은 **자기들끼리 한 조**가 된다 —
  빈 칸 때문에 배정에서 빠지지 않는다는 규칙은 여기서도 같고, 그 조에도 이름이 있어야
  ('학교 미기재') 명단을 받은 사람이 무엇으로 묶인 조인지 안다. 고른 사람 중에 CMU도 Pitt도
  없으면(장년부·EM) 물을 것이 없으므로 그 줄 자체가 뜨지 않는다. **켜면 학교 기준은 빠진다**
  (`ruleForGroup(group, bySchool)`): 조 안의 학교가 하나뿐이라 대학부의 'Pitt/CMU 반반'도
  청년부의 '같은 학교끼리'도 그 주에는 고를 것이 없고, 남겨 두면 화면이 '학교 정보 없음 n명'을
  그 기준의 빈칸으로 계속 적어 준다.
- **`assignEduDongsan`의 `count`는 한 단계를 몇 조로 나눌지이고 기본값이 1이다.** 2 이상으로
  올리면 그 단계 안에서 다시 쪼개지고 **그때 비로소 아래의 기준(성비·나이·학교·전공·신앙)이
  누가 어느 쪽으로 갈지를 정한다** — 조가 하나뿐인 동안에는 고를 것이 없으므로 기준이 아무것도
  바꾸지 않는다. 화면의 숫자 칸이 그 값이다 (1~12).
...
- **조별 명단이 목록 위에 뜨고, 거기서 사람을 옮긴다** (`groupByEduDongsan` + `EduDongsanResult`).
  ...
  ("1동산은 누구누구인가"). 조는 이름 안의 숫자로 세운다 — 글자만으로 세우면 10동산이 2동산

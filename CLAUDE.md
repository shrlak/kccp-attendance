...
- **여기서 새가족은 `members.is_new_member` 하나다** — 새가족 탭의 `visibleNewFamily`가 아니다.
  그쪽은 "지금 새가족팀이 챙길 사람"이라 교육을 마치면 목록에서 빠지는데, **추이는 지나간
  사실**이라 그 사람이 그 달에서 사라지면 이번 주에 지난달 숫자가 바뀐다.
...
- 장년부에는 **새가족 교육 타일이 없고**(그 부에 교육이 없다) "이번 학기"가 "이번 반기"다
  (`nfThisTerm_adult`, `usePartitionT`). 학기 경계는 통계가 정하지 않고 `newFamily.ts`
  `semesterBounds`에서 받아 온다 — 부마다 한 해를 나누는 방식이 다르기 때문.

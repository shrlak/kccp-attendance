...
- **화면이 가리키는 것은 오늘 열면 오늘 것, 아니면 다음에 열리는 것** (`focusEduSession`).
  ...
  "교육이 없어졌나"가 된다.
- **그 주차가 비어 있는 사람이 위 블록으로 올라온다** (`needsEduWeek`): 1주차 주일에는
  미수강 + 2주차만 들은 사람, 2주차 주일에는 미수강 + 1주차만 들은 사람. 한 바퀴가 1주차
  에서만 시작하지 않으므로 **2주차만 들은 사람이 실제로 생긴다** (2주차 주일에 처음 온
  새가족이 그렇다). 이미 들은 사람은 아래 블록에 그대로 남는다 — 이수 체크를 고치는
  자리라 사라지면 방금 찍은 사람이 통째로 없어진 것처럼 보인다.

...
- 위쪽 통계가 전체를 세는 자리라면 그 아래부터는 **새가족만** 센다: 숫자 타일(전체 · 이번 학기
  등록 · 최근 4주 출석 · 새가족 교육 이수), 그래프 둘(**주별** 등록 막대 · 주일별 새가족
  출석 선), 월별 표(등록 · 출석 인원 · 비중). 입력은 위와 **같은** members + log이라 부서/동산 필터가
...
- **등록 막대는 달이 아니라 주다** (`newFamilyRegistrations`, 최근 16주). 주는 **주일에서
  열린다** (`worshipSunday`, 주일→토요일) — 새가족 카드의 '이번 주일 등록/지난주 등록'과 같은
  자름이라 수요일에 옮겨 적은 등록도 그 사람이 온 주일에 앉는다. 달로 묶으면 "8월에 5명"까지만
  보이고 그 다섯이 한 주일에 몰렸는지 넉 주에 흩어졌는지가 사라지는데, 새가족을 맞는 일은 주일
  단위로 돌아간다. 그 앞의 긴 흐름은 아래 **월별 표**가 달 단위로 그대로 들고 있다.
- **축은 이번 주일까지 이어진다** (차트가 `today`를 넘긴다): 마지막 등록에서 축이 끝나 버리면
  몇 주째 아무도 안 온 것이 화면에서 사라진다 (달 단위일 때는 그 침묵이 길어야 눈에 띄었다).
- **등록일이 없는 새가족은 어느 주에도 놓을 수 없어** 그래프·월별 표에서 빠진다. 그래서 그
  인원수를 블록 머리에 적어 둔다 (`nfUndated`) — 안 적으면 타일의 총계와 그래프의 합이 어긋나
  보인다. 같은 이유로 등록이 없던 주도 0으로 채운다 (`weeksBetween`): 건너뛰면 x축이 붙어
  "그 주에도 왔다"로 읽힌다.

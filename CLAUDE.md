...
## 칩 줄 — 부서 · 동산 · 처지 · 학교 (`filters.ts` · `TraitFilter.tsx` · `AdminMembers.tsx` · `AdminNewFamily.tsx`)
- **위에서 아래로 좁혀 간다**: 부서(전체 · 대학부 · 청년부 · 부서 미기재) → **동산**(멤버
  탭에만: 전체 · 그 부서의 동산들 · 동산 미지정) → 처지(청년부에서만: 전체 · 대학원생 ·
  직장인 · 기타) → 학교(전체 · CMU · Pitt · Duquesne · 기타, 청년부에서는
  처지와 곱해진다). 멤버 탭에서는
  이름 검색과 **곱해지고**, 명단·숨긴 멤버·스태프 세 목록에 똑같이 걸린다 — 한 목록만 좁히면
  나머지 목록이 방금 고른 것을 무시하는 것처럼 읽힌다.
- **동산 줄은 고른 부서 안의 동산만 내건다** (`subgroupChipsOf(members, group)`): 청년부를
  누르면 청년부 동산, 대학부를 누르면 대학부 동산, 부서가 '전체'면 둘 다. 동산은 부서 안에
  있으므로 그 줄은 부서 줄 **바로 아래**이고, 부서를 바꾸면 고른 동산은 비운다 (`pickGroup`
  — 그 부서에 있지도 않은 동산으로 좁히고 있으면 화면이 왜 비었는지 알 수가 없다). 처지·학교
  칩은 **동산까지 좁혀진 뒤의** 명단에서 뽑는다. `NO_SUBGROUP`(동산 미지정)은 부서 줄의
  `NO_GROUP`과 같은 이유로 있다 — 칩들의 인원을 더하면 그 부서 전체가 되어야 하고, 학기 종료
  롤오버가 편성을 비운 직후에는 그 묶음이 명단의 대부분이다. 새가족 탭은 예전 그대로 부서·동산
  `GroupFilter`를 쓴다 (같은 일을 하는 줄이 그쪽에 이미 있다).
- **처지·학교 두 줄은 멤버 탭과 새가족 탭이 같은 컴포넌트(`TraitFilter`)로 그린다.** 두 탭이
  ...
  부서·동산 `GroupFilter`. 그래서 `TraitFilter`가 받는 명단은 **부서로 좁혀진 뒤의 것**이다.

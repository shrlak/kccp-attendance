...
## 멤버 탭의 칩 줄 — 부서 · 처지 · 학교 (`filters.ts` · `AdminMembers.tsx`)
- **멤버 탭에만 있는 칩 줄이고, 위에서 아래로 좁혀 간다**: 부서(전체 · 대학부 · 청년부 ·
  부서 미기재) → 처지(청년부에서만: 전체 · 대학원생 · 직장인 · 기타) → 학교(전체 · CMU ·
  Pitt · Duquesne · 기타). 이름 검색과 **곱해지고**, 명단·숨긴 멤버·스태프 세 목록에 똑같이
  걸린다 — 한 목록만 좁히면 나머지 목록이 방금 고른 것을 무시하는 것처럼 읽힌다.
...
- **공용 필터(`Filter`)에는 넣지 않았다.** 출석부·통계·오늘이 세는 것은 그 주일에 누가
  왔는가이고 학교·처지는 그 질문의 칸이 아니다 — 그래서 `filterLog`은 예전 그대로다 (출석 줄에는
  학교가 없으므로, 넣으려면 줄마다 명단을 되짚어야 한다). 명단을 이렇게 갈라 보는 자리는 멤버
  탭 하나다.

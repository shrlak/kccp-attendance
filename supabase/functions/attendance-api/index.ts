...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // 새가족카드 photo scan: extracts the registration fields from a photo of the paper
    // new-family card via Claude vision (structured output) so the kiosk form can be
    // prefilled. No DB writes — registration still goes through /api/admin/kiosk-new-member
    // after a human reviews the extracted fields. Requires the ANTHROPIC_API_KEY function
    // secret (set via `supabase secrets set`); audited.
    if(req.method==="POST"&&p==="/api/admin/scan-new-member-card") {
      const role=await resolveAdmin(sb,req);
      if(!role) return fail(401,"Not authorized");
      if(role.role==="pastor") return fail(403,"Read-only");
      const apiKey=Deno.env.get("ANTHROPIC_API_KEY");
      if(!apiKey) return fail(500,"Card scan not configured (ANTHROPIC_API_KEY secret missing)");
      const imageBase64=typeof body.imageBase64==="string"?body.imageBase64:"";
      const mediaType=typeof body.mediaType==="string"?body.mediaType:"";
      if(!imageBase64||!["image/jpeg","image/png","image/webp"].includes(mediaType)) return fail(400,"imageBase64 and a supported mediaType (jpeg/png/webp) required");
      if(imageBase64.length>8_000_000) return fail(413,"Image too large");
      ...
      const schema={
        ...
        properties:{
          ...
          subgroup:nullableStr("동산 / 소그룹 name"),
          gender:nullableStr("성별 — 남 or 여"),
          phone:nullableStr("전화번호, digits and dashes only, e.g. 412-555-0123"),
          kakaoId:nullableStr("카카오톡 ID"),
          birthDate:nullableStr("생년월일 in YYYY-MM-DD"),
      ...
      try {
        ...
        return fail(502,"Card scan failed: "+((e as Error)?.message||"AI request error"));
      }
      if(msg?.stop_reason==="refusal") return fail(502,"Card scan failed: request was declined");
      const txt=(msg?.content||[]).find((b:any)=>b.type==="text")?.text;
      let f:any={};
      try{f=JSON.parse(txt||"");}catch(_){return fail(502,"Card scan failed: unreadable result");}

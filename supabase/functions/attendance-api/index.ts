...
// Decimal-unit size label, same math as web/src/features/admin/admins.ts formatBytes so
// notification messages and the UI agree on a backup's size.
function fmtBytes(bytes: number): string {
  if(bytes<1000) return bytes+" B";
  const units=["KB","MB","GB","TB"]; let v=bytes/1000,i=0;
  while(v>=1000&&i<units.length-1){v/=1000;i++;}
  return v.toFixed(1)+" "+units[i];
}

// ── Backup completion notifications (config.backup_notifications) ─────────
// Recipients are registered in-app (Admins tab → 전체 백업). Email goes out through
// Resend (RESEND_API_KEY [+ BACKUP_EMAIL_FROM]); 카카오톡 goes out through Solapi 알림톡
// (SOLAPI_API_KEY/SOLAPI_API_SECRET + KAKAO_PF_ID/KAKAO_TEMPLATE_ID), falling back to
// plain SMS via SOLAPI_SENDER when no 알림톡 template is registered. Kakao's API only
// addresses 알림톡 by PHONE NUMBER — a bare 카톡 ID cannot receive API messages, which is
// why recipients are phone numbers. Each unconfigured channel degrades to a named skip
// in the notify response instead of failing the call.
const EMAIL_RE=/^[^\s@]+@[^\s@]+\.[^\s@]+$/;
function normalizePhone(raw: string): string | null {
  const cleaned=raw.replace(/[\s\-().]/g,"");
  return /^\+?\d{9,15}$/.test(cleaned)?cleaned:null;
}
function backupNotifySettings(cfg: any): {enabled:boolean;emails:string[];phones:string[]} {
  const raw=cfg?.backup_notifications;
  const emails=Array.isArray(raw?.emails)?raw.emails.filter((e:any)=>typeof e==="string"&&EMAIL_RE.test(e)):[];
  const phones=Array.isArray(raw?.phones)?raw.phones.map((p:any)=>typeof p==="string"?normalizePhone(p):null).filter((p:any):p is string=>!!p):[];
  return {enabled:raw?.enabled!==false,emails,phones};
}
function emailNotifyConfigured() { return !!Deno.env.get("RESEND_API_KEY"); }
function kakaoNotifyConfigured() {
  if(!Deno.env.get("SOLAPI_API_KEY")||!Deno.env.get("SOLAPI_API_SECRET")) return false;
  return !!(Deno.env.get("KAKAO_PF_ID")&&Deno.env.get("KAKAO_TEMPLATE_ID"))||!!Deno.env.get("SOLAPI_SENDER");
}
// One email to all recipients at once. Returns null on success, else a short reason.
async function sendBackupEmail(to: string[], subject: string, text: string): Promise<string|null> {
  const key=Deno.env.get("RESEND_API_KEY");
  if(!key) return "email-not-configured";
  const from=Deno.env.get("BACKUP_EMAIL_FROM")||"KCCP 출석 <onboarding@resend.dev>";
  const res=await fetch("https://api.resend.com/emails",{method:"POST",headers:{"Authorization":"Bearer "+key,"Content-Type":"application/json"},body:JSON.stringify({from,to,subject,text})});
...
async function solapiAuthHeader(apiKey: string, apiSecret: string): Promise<string> {
  ...
  const hmacKey=await crypto.subtle.importKey("raw",new TextEncoder().encode(apiSecret),{name:"HMAC",hash:"SHA-256"},false,["sign"]);
  const sig=await crypto.subtle.sign("HMAC",hmacKey,new TextEncoder().encode(date+salt));
  const signature=Array.from(new Uint8Array(sig)).map((b)=>b.toString(16).padStart(2,"0")).join("");
  return "HMAC-SHA256 apiKey="+apiKey+", date="+date+", salt="+salt+", signature="+signature;
}
// 알림톡 when a channel + approved template are configured (the template must use the
// #{time} / #{size} variables), otherwise plain SMS from the registered sender number.
async function sendBackupKakao(phones: string[], text: string, variables: Record<string,string>): Promise<{sent:number;skipped:string|null}> {
  const apiKey=Deno.env.get("SOLAPI_API_KEY"),apiSecret=Deno.env.get("SOLAPI_API_SECRET");
  if(!apiKey||!apiSecret) return {sent:0,skipped:"kakao-not-configured"};
  const pfId=Deno.env.get("KAKAO_PF_ID"),templateId=Deno.env.get("KAKAO_TEMPLATE_ID"),sender=Deno.env.get("SOLAPI_SENDER")||"";
  const alimtalk=!!(pfId&&templateId);
  if(!alimtalk&&!sender) return {sent:0,skipped:"kakao-not-configured"};
  const messages=phones.map((to)=>alimtalk
    ?{to,...(sender?{from:sender}:{}),kakaoOptions:{pfId,templateId,variables,disableSms:!sender}}
    :{to,from:sender,text});
  const res=await fetch("https://api.solapi.com/messages/v4/send-many/detail",{
    method:"POST",
    headers:{"Authorization":await solapiAuthHeader(apiKey,apiSecret),"Content-Type":"application/json"},
    body:JSON.stringify({messages}),
  });
  if(!res.ok) return {sent:0,skipped:"solapi "+res.status+": "+(await res.text()).slice(0,200)};
  const data=await res.json().catch(()=>null);
  const failed=Array.isArray(data?.failedMessageList)?data.failedMessageList.length:0;
  return {sent:Math.max(0,phones.length-failed),skipped:failed?failed+" kakao message(s) failed":null};
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // Backup-completion notification recipients — read (super-admin only). The
    // *Configured flags report whether each channel's provider secrets are set on the
    // server, so the UI can distinguish "no recipients" from "can't deliver".
    if(req.method==="GET"&&p==="/api/admin/db-backup/notifications") {
      const role=await resolveAdmin(sb,req);
      if(role?.role!=="super_admin") return fail(403,"Super admin required");
      const cfgRow=await getCfg(sb);
      const {enabled,emails,phones}=backupNotifySettings(cfgRow);
      return ok({enabled,emails,phones,emailConfigured:emailNotifyConfigured(),kakaoConfigured:kakaoNotifyConfigured()});
    }

    // Backup-completion notification recipients — write (super-admin only). Every
    // address/number is validated and deduped, and the lists are capped so a mistake
    // can't turn the notifier into a bulk sender. Audited as a config-change.
    if(req.method==="POST"&&p==="/api/admin/db-backup/notifications") {
      const role=await resolveAdmin(sb,req);
      if(role?.role!=="super_admin") return fail(403,"Super admin required");
      const {enabled,emails,phones}=body;
      if(!Array.isArray(emails)||!Array.isArray(phones)) return fail(400,"emails and phones arrays required");
      if(emails.length>20||phones.length>20) return fail(400,"Too many recipients (max 20 each)");
      const cleanEmails: string[]=[];
      for(const e of emails){
        if(typeof e!=="string"||!EMAIL_RE.test(e.trim())) return fail(400,"Invalid email: "+String(e).slice(0,80));
        const v=e.trim().toLowerCase(); if(!cleanEmails.includes(v)) cleanEmails.push(v);
      }
      const cleanPhones: string[]=[];
      for(const ph of phones){
        const v=typeof ph==="string"?normalizePhone(ph):null;
        if(!v) return fail(400,"Invalid phone: "+String(ph).slice(0,40));
        if(!cleanPhones.includes(v)) cleanPhones.push(v);
      }
      const {error}=await sb.from("config").update({backup_notifications:{enabled:enabled!==false,emails:cleanEmails,phones:cleanPhones},updated_at:new Date().toISOString()}).eq("id",1);
      if(error) throw new Error(error.message);
      await addAudit(sb,"config-change",xDev,"백업 알림 수신자 수정 (이메일 "+cleanEmails.length+" · 카카오톡 "+cleanPhones.length+")");
      return ok({status:"ok"});
    }

    // Called by the backup workflow's last step (scripts/backup/notify.sh) once the new
    // current.* set is verified in R2. Authenticated by config.backup_notify_token — a
    // machine token minted by migration; the workflow reads it over its existing
    // read-only backup_reader connection, so no extra GitHub secret is involved. Sends
    // the registered recipients an email + 카카오톡 and reports what was skipped and why.
    if(req.method==="POST"&&p==="/api/admin/db-backup/notify") {
      const cfgRow=await getCfg(sb);
      const token=(cfgRow as any).backup_notify_token;
      if(!token||typeof token!=="string") return fail(500,"backup_notify_token missing — apply the backup_notifications migration");
      if((req.headers.get("x-backup-token")||"")!==token) return fail(401,"Invalid backup token");
      const {enabled,emails,phones}=backupNotifySettings(cfgRow);
      if(!enabled) return ok({status:"disabled",emailsSent:0,kakaoSent:0,skipped:["notifications-disabled"]});
      // Freshest current.* metadata for the message body; best-effort — the notification
      // still goes out (without a size) if the R2 listing hiccups.
      let sizeLabel="",timeLabel=new Date().toLocaleString("ko-KR",{timeZone:"America/New_York",dateStyle:"medium",timeStyle:"short"});
      const s3=r2Client();
      if(s3){try{
        const listing=await s3.send(new ListObjectsV2Command({Bucket:r2Bucket(),Prefix:"backups/current."}));
        let total=0; let last: Date|undefined;
        for(const o of listing.Contents||[]){total+=o.Size||0;if(o.LastModified&&(!last||o.LastModified>last))last=o.LastModified;}
        if(total) sizeLabel=fmtBytes(total);
        if(last) timeLabel=last.toLocaleString("ko-KR",{timeZone:"America/New_York",dateStyle:"medium",timeStyle:"short"});
      }catch(_){}}
      const subject="✅ KCCP 출석 시스템 백업 완료"+(sizeLabel?" ("+sizeLabel+")":"");
      const text="KCCP 출석 시스템의 전체 암호화 백업이 완료되었습니다.\n\n완료 시각 (피츠버그): "+timeLabel+(sizeLabel?"\n백업 크기: "+sizeLabel:"")+"\n\n백업 관리: https://shrlak.github.io/kccp-attendance/admin (관리자 탭 → 전체 백업)";
      const skipped: string[]=[]; let emailsSent=0,kakaoSent=0;
      if(!emails.length) skipped.push("no-email-recipients");
      else {const err=await sendBackupEmail(emails,subject,text);if(err)skipped.push(err);else emailsSent=emails.length;}
      if(!phones.length) skipped.push("no-kakao-recipients");
      else {const kk=await sendBackupKakao(phones,text,{"#{time}":timeLabel,"#{size}":sizeLabel||"-"});kakaoSent=kk.sent;if(kk.skipped)skipped.push(kk.skipped);}
      await addAudit(sb,"db-backup-notify","backup-workflow","이메일 "+emailsSent+"명 · 카카오톡 "+kakaoSent+"명"+(skipped.length?" · 건너뜀: "+skipped.join(", "):""));
      return ok({status:"ok",emailsSent,kakaoSent,skipped});
    }
